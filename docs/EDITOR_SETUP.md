...
Папка `Game` (та, где лежит этот файл) уже содержит `Assets/_Project` с
кодом, `.git`, `.gitignore` и документацию — но ещё не содержит
`ProjectSettings/` и `Packages/`, которые создаёт сама Unity.

1. Unity Hub → вкладка `Projects` → кнопка `New project`.
2. Editor version: **6000.3.x (LTS)**.
3. Шаблон: **2D (URP)** (первый в списке 2D-шаблонов — какой именно рендер
   выбран, для этого проекта не принципиально).
4. **Location**: `C:\Users\yashi\OneDrive\Desktop` (папка ВЫШЕ `Game`).
5. **Project name**: `Game` — вместе с Location это укажет ровно на нашу
   существующую папку.
6. Если Unity предупредит, что папка не пустая — это нормально и ожидаемо
   (там уже наш код и git), можно продолжать.
7. `Create project`. Первое открытие займёт несколько минут — Unity
   импортирует все `.cs`-файлы и создаёт свою служебную папку `Library/`.

## 2. TextMeshPro

Почти весь текст в проекте использует TextMeshPro (TMP) — более гибкий
инструмент текста, чем старый Unity `Text`. Unity предложит установить его
ресурсы при первом использовании:

- `Window → TextMeshPro → Import TMP Essential Resources` → `Import`.

Если не предложило само — сделайте этот шаг вручную сейчас, до создания
объектов с текстом.
